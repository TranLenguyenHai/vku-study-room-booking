...
* **Student Information:**
  ...
  - **Vai trò & Đóng góp:** Toàn quyền thực hiện (100% Contribution - Full-stack Architecture, UI/UX, State Management & Native Features).
...
| **4** | **Global State Management (Zustand & AsyncStorage)** | ✅ Complete | Quản lý phiên sinh viên, danh mục phòng học, danh sách đặt phòng và hành động hủy qua `useBookingStore`. Tích hợp middleware `persist` với `@react-native-async-storage/async-storage` duy trì dữ liệu bền vững qua các lần tắt/mở app. Hủy phòng lập tức giải phóng slot. |
| **5** | **Local Notifications (expo-notifications)** | ✅ Complete | Đăng ký quyền thông báo hệ thống, tự động lập lịch báo thức trước **15 phút** so với thời điểm ca học bắt đầu. Cung cấp nút **"Bắn thông báo thử nghiệm"** trực tiếp trên vé để test tức thì trên iOS. |
...
├── package.json                     # Dependencies (Zustand, React Navigation, Async Storage)
...
└── src/
    ...
    ├── store/
    │   ├── useBookingStore.ts       # Zustand Store + AsyncStorage persist & Conflict Engine
    ...
    │   └── ProfileScreen.tsx        # Hồ sơ sinh viên Trần Lê Nguyên Hải (23IT.EB031)
    └── navigation/AppNavigator.tsx  # Bottom Tabs Navigator & Native Stack
```

### 3.2. State Management & Real-time Conflict Engine Flow
- **Nguyên lý Conflict Engine**: Store lưu mảng `reservations: Booking[]`. Hàm `isSlotBooked(roomId, date, slotId)` duyệt các bản ghi có trạng thái khác `CANCELLED`. Nếu slot đã tồn tại, nút bấm chuyển sang `disabled`, hiển thị tên sinh viên đã giữ chỗ.
- **Hủy lịch tức thì**: Khi sinh viên bấm Hủy trong tab *Lịch Đặt*, trạng thái chuyển thành `CANCELLED`, ngay lập tức giải phóng khung giờ trong bộ nhớ toàn cục mà không cần tải lại app.
...
|:---:|:---:|
| Mã QR Vector SVG tạo động, hiển thị mã `VKU-BK-XXXXX`, nút mô phỏng Quét Check-in và Bắn thông báo 15p. | Quản lý danh sách ca đã đặt, nút Hủy phòng giải phóng slot tức thì, thẻ sinh viên Trần Lê Nguyên Hải (23IT.EB031). |
